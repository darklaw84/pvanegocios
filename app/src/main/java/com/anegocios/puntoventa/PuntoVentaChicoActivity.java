...
public class PuntoVentaChicoActivity extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener {
    ...
    private void mostrarPuntoVenta() {
        setContentView(R.layout.puntoventachico);
        ...
        if (estatusImpresora) {
            ...
            botonImpresora.setImageResource(R.drawable.bluetoothnook);
        }
    ...
    public void btnCodCamaraClick(View view) {
        ...
        new IntentIntegrator(PuntoVentaChicoActivity.this).initiateScan();
    }
    ...
    public void btnGuardarExtrasClick(View view) {
        ...
        calculaMontosLista();
        if (vengode.equals("carritocompras")) {
            mostrarCarritoCompras();
        } else {
            mostrarPuntoVenta();
        }
    ...
    private void agregarProducto(ProductosXYDTOAux p, boolean llenandoPedido) {
        try {
            ...
            if (p.isPaquete()) {
                ...
                if (p.getCantidadMayoreo() > 0) {
                    //quiere decir que lo tenemos que evaluar
                    if (p.getCantidad() >= p.getCantidadMayoreo()) {
                        p.setPrecioVenta(p.getPrecioMayoreo());
                    }
                }
                agregarSolito(p);
  ...
  /*  private void actualizarTotalesProductos() {
    ...
    public void btnCobrarClick(View view) {
        ...
        if (esEditarPedido) {
            ...
            if (productosAgregados != null && productosAgregados.size() > 0) {
                Utilerias ut = new Utilerias();
                setContentView(R.layout.pantallapagar);
...
*/
    ...
    private void mostrarCarritoCompras() {
        ...
        pantalla = "carritocompras";
        vengode = "carritocompras";
        iniciarMenuContextual();
        ...
        ProductosAgregadosAdapter adapter = new ProductosAgregadosAdapter(productosAgregados, this, "C");
        gvProductosAgregados.setAdapter(adapter);
    ...
    public void btnCarritoClick(View view) {
        if (productosAgregados != null && productosAgregados.size() > 0) {
            mostrarCarritoCompras();
        } else {
            mandarMensaje("Debes agregar algun producto para poder generar la venta");
    ...
    private void llenarComboGrupos() {
        ...
        ArrayAdapter aa = new ArrayAdapter(this, android.R.layout.simple_spinner_item, gruposAux);
